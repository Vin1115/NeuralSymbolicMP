...
class Reasoner:
    ...
    def get_free_vec(self,
                    term_name,
                    begin_index=None,
                    end_index=None):
        if self.term_local_vector_dict[term_name] is not None:
            vec = self.term_local_vector_dict[term_name]
        else:
            raise KeyError("Free v's vector does not found")
        return vec
    ...
    def get_rel_emb(self,
                    ...
                    end_index=None,
                    inv=False):
        relation_idx = self.formula.get_pred_grounded_relation_id_list(rel_name)
        emb = self.nbp.get_relation_emb(relation_idx, inv)
        
        if(inv): 
            for i in range(len(relation_idx)):
                if(relation_idx[i]%2!=0):
                    relation_idx[i] -= 1
                else:
                    relation_idx[i] += 1
                    
        relation_idx_tensor = torch.tensor(relation_idx, dtype=torch.int64).to(self.nbp.device)

        if begin_index is not None and end_index is not None:
            emb = emb[begin_index: end_index]
        return emb, relation_idx_tensor
