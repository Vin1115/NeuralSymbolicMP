...
class NeuralSymbolicMPLayer(nn.Module):
    def __init__(self, nbp: NeuralBinaryPredicate, mat=None, alpha=100):
        super(NeuralSymbolicMPLayer, self).__init__()
        self.nbp = nbp
        ...
        self.existential_embedding = torch.zeros((1, self.feature_dim)).to(self.nbp.device)
        self.universal_embedding = torch.zeros((1, self.feature_dim)).to(self.nbp.device)
        self.free_embedding = torch.zeros((1, self.feature_dim)).to(self.nbp.device)
    ...
    def message_passing(self,
        ...
        for predicate, atomic in atomic_dict.items():
            ...
            if tail_name in ['f', 'e1', 'e2', 'e3', 'f1', 'f2']: 
                if torch.all(head_emb[0] == 0): 
                    term_collect_vectors_dict[tail_name].append('null')
                else:
            ...
            if head_name in ['f', 'e1', 'e2', 'e3', 'f1', 'f2']:  
                if torch.all(tail_emb[0] == 0): 
                    term_collect_vectors_dict[head_name].append('null')
                else:
    ...
    def forward(self, 
        ...
        for t, collect_vector_list in term_collect_vectors_dict.items(): 
            ...
            new_tensor_list = [item for item in collect_vector_list if isinstance(item, torch.Tensor)]
            if len(new_tensor_list) != 0:
                agg_vec = self.product_conjunction(new_tensor_list)
            else:
                agg_vec = init_term_vector_dict[t]
...
class NeuralSymbolicMPReasoner(Reasoner):
    def __init__(self,
                 nbp: NeuralBinaryPredicate,
                 lgnn_layer: NeuralSymbolicMPLayer,
                 depth_shift=0
                 ):
        self.nbp = nbp
